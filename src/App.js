...
const questions = [
    {
        ...
        options: {
            option1: " 111 option1 sit amet consectetur 111 option1 sit amet consectetur 111 option1 sit amet consectetur 111 option1 sit amet consectetur adipisicing elit.  Cumoluptate! Delectus placeat i",
...
function colorUncheckRadios() {
    ...
    answerOptions.forEach((radio) => {
        if (!radio.checked) {
            let tempParent = radio.parentElement;
        ...
        case "multiSelect": {
            ...
            result = optionsList
                ...
                return `<div  onClick="checkInput('${index}div',${index})" id="${index}div" class="answerOption qCard">
                    <input type='${question.inputType}' name="Answer" id='${index}' value='${optionTitle}' />
                    <label for=${specifyLabelForString(question.inputType, index)} id="option-label-${index + 1}">${optionTitle}</label>
                </div>`;
            })
                .join("");
            break;
